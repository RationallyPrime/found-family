...
from memory_palace.infrastructure.neo4j.driver import open_neo4j_driver
...
def calculate_salience(_tier: str, metadata: dict) -> float:
...
async def import_tier_memories(
    tier_dir: Path, tier_name: str, memory_service: MemoryService, batch_size: int = 5
) -> tuple[int, int]:
...
async def main() -> int:
    ...
    parser.add_argument("--batch-size", type=int, default=5, help="Number of memories to process at once")
    parser.add_argument("--dry-run", action="store_true", help="Analyze files but don't import")
    ...
    # Initialize services
    driver = await open_neo4j_driver()
        ...
        async with driver.session() as session:
            ...
            for tier in args.tiers:
                ...
                imported, skipped = await import_tier_memories(
                    tier_dir, tier, memory_service, batch_size=args.batch_size
                )
