...
def parse_conversation_file(file_path: Path) -> list[dict[str, Any]]:
    ...
    for human, assistant in zip(humans, assistants, strict=False):
        turns.append({"user": human.strip(), "assistant": assistant.strip(), "timestamp": None, "metadata": {}})
...
async def main() -> int:
    ...
    parser.add_argument("source_dir", type=Path, help="Directory containing conversation files")
    parser.add_argument(
        "--highlighted",
        action="store_true",
    ...
    parser.add_argument("--dry-run", action="store_true", help="Parse files but don't import to database")
        ...
        async with driver.session() as session:
            ...
            for file in files:
                ...
                imported, skipped = await import_conversation(
                    file, memory_service, conversation_id=conv_id, is_highlighted=args.highlighted
                )
