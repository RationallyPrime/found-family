...
async def _create_test_memory(session, *, salience: float, updated_days_ago: float, pinned: bool = False, last_accessed_days_ago: float | None = None) -> str:
    mid = str(uuid.uuid4())
    await session.run(
        """
...
async def _get(session, mid: str) -> dict:
...
async def test_decay_matches_formula(neo4j_session):
    ...
    await neo4j_session.run(
        query, now=NOW, decay_lambda=SALIENCE_DECAY_LAMBDA_PER_DAY, floor=SALIENCE_FLOOR
    )
...
async def test_decay_is_cadence_independent_in_cypher(neo4j_session):
...
async def test_pinned_memories_never_decay(neo4j_session):
...
async def test_reinforcement_boosts_and_tracks_access(neo4j_session):
...
async def test_archive_only_takes_stale_unpinned_low_salience(neo4j_session):
    stale = await _create_test_memory(
        neo4j_session, salience=0.06, updated_days_ago=100.0, last_accessed_days_ago=100.0
    )
    fresh = await _create_test_memory(
        neo4j_session, salience=0.06, updated_days_ago=100.0, last_accessed_days_ago=5.0
    )
...
async def test_spread_activation_through_typed_edges(neo4j_session):
    ...
    result = await neo4j_session.run(
        query, seeds=[{"id": a, "score": 1.0}], hop_decay=0.7, limit=10
    )
    activations = {}
...
async def test_archived_memories_excluded_from_spread_activation(neo4j_session):
    a = await _create_test_memory(neo4j_session, salience=0.5, updated_days_ago=0.0)
    b = await _create_test_memory(neo4j_session, salience=0.5, updated_days_ago=0.0)
    await neo4j_session.run(
        """
