...
class EmotionalMemorySpecification(BaseSpecification):
    ...
    def is_satisfied_by(self, entity: Any) -> bool:
        ...
        return (entity.emotional_intensity >= self.min_intensity and 
                self.valence_range[0] <= entity.emotional_valence <= self.valence_range[1])
    ...
    def to_cypher(self) -> str:
        return (f"m.emotional_intensity >= {self.min_intensity} AND "
...
class DecayingMemorySpecification(BaseSpecification):
    ...
    def to_filter(self) -> dict[str, Any]:
        return {
            "$or": [
                {"last_accessed": None},
                {"last_accessed__lt": self.cutoff.isoformat()}
            ],
